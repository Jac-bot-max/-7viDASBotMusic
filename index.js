...
app.get('/', (req, res) => res.send('@7viDASBotMusic Online e Automático!'));
app.listen(port, '0.0.0.0', () => console.log(`✅ Servidor na porta ${port}`));

// MEMÓRIA GLOBAL DE ADVERTÊNCIAS
if (!global.advertencias) global.advertencias = new Map();

// REGRAS DO GRUPO
const REGRAS = `📜 *REGRAS JACKSON@7VIDAS*
1️⃣ Proibido links e mídias fora do tema.
2️⃣ Proibido menções de Status ou Spam.
3️⃣ Proibido Bullying ou Insultos.
...
async function startBot() {
            ...
            for (const num of participants) {
                try {
                    ...
                    await socket.sendMessage(id, { 
                        image: { url: foto }, 
                        caption: `👋 Olá @${num.split('@')[0]}!\nBem-vindo ao grupo de Produção!\n\n${REGRAS}`, 
                        mentions: [num] 
                    });
    ...
    socket.ev.on("messages.upsert", async (m) => {
        try {
            ...
            // --- LÓGICA DE ÁUDIO (CÉREBRO INTELIGENTE) ---
            if (type === 'audioMessage') {
                const isPTT = msg.message.audioMessage.ptt; // true = gravação de voz / false = arquivo/beat
                if (isPTT) {
                    // SE FOR GRAVAÇÃO DE VOZ: APENAS REAGE
                    await socket.sendMessage(from, { react: { text: "🎙️", key: msg.key } });
                } else {
                    // SE FOR ARQUIVO (BEAT/INSTRUMENTAL): REAGE E MANDA A FRASE NOVA
                    await socket.sendMessage(from, { react: { text: "✅", key: msg.key } });
                    if (isGroup) {
                        await socket.sendMessage(from, { 
                            text: "⚪ *[ @7viDASBotMusic ]* ⚪\n\n🔵 _Positivo, aguarde. Um dos seus amigos (beatmakers) vai analisar esta obra._" 
                        }, { quoted: msg });
                    }
                }
                return;
            ...
            // RESPOSTA NO PRIVADO (DM)
            if (!isGroup) {
                if (textRaw.startsWith('!')) {
                    if (textRaw.includes('ping')) await socket.sendMessage(from, { text: "🚀 @7viDASBotMusic está online no privado!" });
                }
                return;
            }

            // --- FUNÇÕES DE ADMINISTRAÇÃO AUTOMÁTICA ---
            ...
            if (isBotAdmin && !isSenderAdmin) {
                ...
                // Detectar se citaram "status" e marcaram pessoas (Spam de Status)
                ...
                if (linkRegex.test(textRaw) || isMidia || isSpamStatus) {
                    await socket.sendMessage(from, { react: { text: "❌", key: msg.key } });
                    await delay(800);
                    await socket.sendMessage(from, { delete: msg.key }); // APAGA SOZINHO
                    ...
                    if (avisos >= 3) {
                        ...
                        await socket.sendMessage(from, { text: `🔴 *BANIDO:* @${sender.split('@')[0]} expulso por ignorar os avisos automáticos.`, mentions: [sender] });
                        global.advertencias.delete(sender);
                    } else {
                        await socket.sendMessage(from, { text: `⚠️ *AVISO [${avisos}/3]* @${sender.split('@')[0]}.\nProibido links, mídia fora do tema ou spam de status!`, mentions: [sender] });
                    }
                    return;
                }
            }
